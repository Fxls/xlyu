...
import com.itek.servlet.day04.dao.impl.AccountImpl;
import com.itek.servlet.day04.dao.inter.AccountInter;
...
public class AccountServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch (uri) {
            //增
            case "/servletDay04/account/add.do":
                //将输入的数据传入数据库
                /**
                 * 1、jdbc配置
                 * 2、DAO的创建
                ...
                resp.sendRedirect("http://localhost:8080/servletDay04/account/select.do");


                break;
            //删
            case "/servletDay04/account/delete.do":
                //执行删除
                ...
                break;
            //查
            case "/servletDay04/account/select.do":
                ...
                for (Account account : list) {
                    sb.append("    <tr>\n" +
                            ...
                            "        <td><a href='http://localhost:8080/servletDay04/account/delete.do?account_id=" + account.getAccount_id() + "'><input type='button' value='删除'></a>&nbsp;" +
                            "<a href='http://localhost:8080/servletDay04/account/change.do?account_id=" + account.getAccount_id() + "&real_name=" + account.getReal_name() + "&login_name=" + account.getLogin_name() + "&telephone=" + account.getTelephone() + "&idcard_no=" + account.getIdcard_no() + "&status=" + account.getStatus() + "'><input type='button' value='修改'></a>" +
                            "</td>\n" +
                            "    </tr>\n");
                }
                ...
                sb.append("    </tbody>\n" +
                        "</table>" +
                        "<a href='http://localhost:8080/servletDay04/html/account.html'><button>新增</button></a>" +
                        "</body>\n" +
                        "</html>");
                ...
                break;
            //改
            case "/servletDay04/account/change.do":
...
////                Integer idGet = Integer.parseInt(account_id2);
                ...
                pw2.println("<!DOCTYPE html>\n" +
                        ...
                        "    <form action='http://localhost:8080/servletDay04/account/changeSure.do'>\n" +
                        "        <span>用户ID（无法修改）：</span><input style='outline:none;' readonly type=\"text\" name='account_id' value='" + account_id2 + "'><br/>\n" +
                        "        <span>用户登录名：</span><input type=\"text\" name='login_name'  value='" + login_name2 + "'><br/>\n" +
                        "        <span>真实姓名：</span><input type=\"text\" name='real_name'  value='" + real_name2 + "'><br/>\n" +
                        "        <span>手机号：</span><input type=\"text\" name='telephone'  value='" + telephone2 + "'><br/>\n" +
                        "        <span>身份证号：</span><input type=\"text\" name='idcard_no'  value='" + idcard_no2 + "'><br/>\n" +
...
//                        "<button onclick=\"f();\">确定</button>"+
                ...
                break;
            case "/servletDay04/account/changeSure.do":
