 ...
 **/
public class TetriminoZ extends Tetrimino implements Serializable {
    //表示两种状态的方块组合
    private static final int TRANSVERSE = 1;
    private static final int VERTICAL = 2;
    private static final long serialVersionUID = 4826168982050844298L;
    //接收方块状态的变量
    private int state;

    public TetriminoZ(int row, int col, Image backImage) {
    ...
    public void rotate(boolean dir) {
        switch (state) {
            case TRANSVERSE:
                ...
                cells[3] = new Cell(cells[0].row + 1, cells[0].col - 1, backImage);
                state = VERTICAL;
                break;
            case VERTICAL:
