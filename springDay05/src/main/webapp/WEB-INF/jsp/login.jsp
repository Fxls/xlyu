...
<div style="position: absolute" class="main">
    <form action="login.do" method="post" onsubmit="return check();">
        ...
        <div class="username"><input class="userText" id="nameId" type="text" placeholder="支持QQ号/手机号/邮箱登录"
                                     name="adminCode" value="${loginName}"/><span></span></div>
        <div class="pwd"><input class="pwdText" id="pwdId" type="password" placeholder="QQ密码" name="password"
        ...
        &nbsp;<input class="remember" type="text" name="cap" placeholder="验证码" id="cb"/><img
            style="position: absolute;margin-top: 5px;margin-left: 10px" src="<%=request.getContextPath()%>/cap.do"
            onclick="this.src='<%=request.getContextPath()%>/cap.do?d='+Math.random();"><span style="position: absolute;margin-left: 130px;margin-top: 10px;font-size: 30px;font-weight: bolder;color: #ff1b54;">${msg}</span><br/>
